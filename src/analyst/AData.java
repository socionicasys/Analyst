...
public String getSecondAspect(){return secondAspect;}

public void setSign(String s) throws ADataException{  	
	if (s == null)  {sign = null; return;}  
	if (!( s.equals(PLUS)
		  |s.equals(MINUS)
...
public void setDimension(String s) throws ADataException{
    if (s == null)  {dimension = null; return;} 
	if (  !( s.equals(D1)
			|s.equals(D2)
			|s.equals(D3)
			|s.equals(D4)
...
public String toString(){
	...
	String res =aspect;				//	=  aspect + separator;
		if (modifier!= null && modifier.equals(BLOCK)) {
			...
			if (modifier!= null && modifier.equals(JUMP)) {
...
public static AData parceAData(String s) throws ADataException{
	...
	//detecting aspect
		...
		if(s.contains(P)) aspect = P;
		else
		if(s.contains(R)) aspect = R;
		else
		if(s.contains(E)) aspect = E;
		else
		if(s.contains(S)) aspect = S;
		else
		if(s.contains(F)) aspect = F;
		else
		if(s.contains(T)) aspect = T;
		else
		if(s.contains(I)) aspect = I;
		else
		if(s.contains(I)) aspect = I;	
	...
	if(s.contains(MENTAL))	 mv = MENTAL;
		...
		if(s.contains(VITAL))		 mv = VITAL;
		else
			if(s.contains(SUPEREGO))		 mv = SUPEREGO;
			else
				if(s.contains(SUPERID))		 mv = SUPERID;
	
	//detecting dimension
	if(s.contains(D1))	 dimension = D1;
	else
		if(s.contains(D2))		 dimension = D2;	
		else
			if(s.contains(D3))		 dimension = D3;	
			else
				if(s.contains(D4))		 dimension = D4;	
				else
	...
	if(s.contains(PLUS))	 sign = PLUS;
		...
		if(s.contains(MINUS))		 sign = MINUS;	
